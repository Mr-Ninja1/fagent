...
import { FaSearch, FaPlus, FaDumbbell } from 'react-icons/fa';
...
const Gyms = () => {
  ...
  useEffect(() => {
    const fetchGyms = async () => {
      try {
        // Fetch only teams created by instructors (gyms)
        const response = await teamAPI.getTeams({
          type: 'gym' // Filter by type 'gym'
        });
        ...
        {filteredTeams.map(team => (
          <Link to={`/teams/${team.id}`} key={team.id} style={teamCardStyle}>
            <div style={teamImageContainerStyle}>
              <div style={teamImageContainerContainerStyle}>
                <img 
                  src={`http://localhost:5000${team.logo || '/default-gym-logo.png'}`} 
                  alt={team.name}
            ...
            <div style={teamInfoStyle}>
              ...
              {team.owner && <p style={ownerNameStyle}>Instructor: {team.owner.username}</p>}
...
const teamImageContainerContainerStyle = {
  width: '100%',
  height: '100%',
  overflow: 'hidden',
};
